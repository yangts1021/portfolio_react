...
const App: React.FC = () => {
  ...
  const [alertConfig, setAlertConfig] = useState<{ title: string; message: string; json?: any } | null>(null);
  ...
  useEffect(() => { localStorage.setItem(STORAGE_KEYS.BANK, JSON.stringify(bankData)); }, [bankData]);
  useEffect(() => { localStorage.setItem(STORAGE_KEYS.PLEDGE, JSON.stringify(pledgeData)); }, [pledgeData]);
  useEffect(() => { localStorage.setItem(STORAGE_KEYS.PRICES, JSON.stringify(currentPrices)); }, [currentPrices]);
  useEffect(() => { localStorage.setItem(STORAGE_KEYS.BETAS, JSON.stringify(symbolBetas)); }, [symbolBetas]);
  useEffect(() => { localStorage.setItem(STORAGE_KEYS.GAS_URL, gasUrl); }, [gasUrl]);
  useEffect(() => { localStorage.setItem(STORAGE_KEYS.RATES, JSON.stringify(exchangeRates)); }, [exchangeRates]);
  useEffect(() => { localStorage.setItem(STORAGE_KEYS.RATE_MODE, rateMode); }, [rateMode]);
  ...
  const fetchDataFromGAS = useCallback(async (isSilent = false) => {
    if (!gasUrl) {
      if (!isSilent) setAlertConfig({ title: '提示', message: '請先輸入 Google Apps Script 網址' });
      return;
    }
    ...
    try {
      ...
      if (json.transactions) {
        const formattedTx = json.transactions.map((row: any, index: number) => ({
          ...
          action: (row.action === '賣' || row.action === 'SELL' || row.action === 'S') ? 'SELL' : 'BUY',
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-gray-50 dark:bg-gray-950 transition-colors duration-300">
      ...
      <div className="fixed bottom-5 right-5 z-50 flex flex-col gap-2">
        {toasts.map((toast, i) => (
          <div key={i} className="bg-gray-800 dark:bg-gray-700 text-white px-4 py-2 rounded shadow-lg text-sm animate-bounce">
            {toast}
          </div>
        ))}
      </div>
