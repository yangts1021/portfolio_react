    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-white dark:bg-gray-900 rounded-xl shadow-2xl p-6 animate-fade-in border border-gray-100 dark:border-gray-800 transition-colors">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 text-left whitespace-pre-wrap bg-gray-50 dark:bg-gray-800 p-3 rounded border border-gray-100 dark:border-gray-700">{config.message}</p>
          <button onClick={onClose} className="w-full bg-gray-800 dark:bg-blue-600 text-white font-medium rounded-lg py-2.5 hover:bg-gray-900 dark:hover:bg-blue-700 transition-colors">關閉</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="relative w-full max-w-sm bg-white dark:bg-gray-900 rounded-xl shadow-2xl p-6 animate-fade-in border border-gray-100 dark:border-gray-800 transition-colors">
        <div className="text-center">
          ...
          <div className="flex gap-3">
            <button onClick={onClose} className="flex-1 bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-300 rounded-lg py-2.5 hover:bg-gray-300 dark:hover:bg-gray-700 transition-colors">取消</button>
            <button onClick={() => { config.onConfirm(); onClose(); }} className="flex-1 bg-blue-600 text-white rounded-lg py-2.5 hover:bg-blue-700 transition-colors">確定</button>
          </div>
        </div>
      </div>
    </div>
...
export const DataSyncModal: React.FC<DataSyncModalProps> = ({ isOpen, onClose, gasUrl, setGasUrl, onFetch, showToast }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-gray-900 rounded-xl shadow-2xl overflow-hidden animate-fade-in border border-gray-100 dark:border-gray-800 transition-colors">
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-gray-800/50">
          ...
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 transition-colors"><i className="fa-solid fa-xmark text-xl"></i></button>
        </div>
        <div className="p-6 space-y-6">
          <div className="space-y-3">
            <h4 className="text-sm font-bold text-gray-700 dark:text-gray-300 border-l-4 border-green-500 pl-2">Google Apps Script 網址</h4>
            <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">請貼上已部署的 Google Apps Script 網頁應用程式網址，用於讀取交易、價格、銀行餘額及質押紀錄。</p>
            <input 
              type="url" 
              value={gasUrl} 
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={() => { localStorage.setItem('my_gas_url', gasUrl); showToast('已儲存設定'); }} className="flex-1 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-lg py-2 text-sm border border-gray-200 dark:border-gray-700 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">儲存</button>
              <button 
                onClick={async () => { setLoading(true); await onFetch(false); setLoading(false); }} 
