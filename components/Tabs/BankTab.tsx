...
const BankTab: React.FC<BankTabProps> = ({ bankData, setBankData, exchangeRates, gasUrl, showToast, onRefresh }) => {
    ...
    <div className="animate-fade-in space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-gray-900 p-5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col items-center justify-center text-center transition-colors">
          <div className="text-gray-400 dark:text-gray-500 text-xs font-medium uppercase tracking-wider mb-1">銀行總餘額 (USD)</div>
        ...
        {/* 上方總額摘要卡片 - TWD 總額 */}
        <div className="bg-white dark:bg-gray-900 p-5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col items-center justify-center text-center transition-colors">
          <div className="text-gray-400 dark:text-gray-500 text-xs font-medium uppercase tracking-wider mb-1">銀行總餘額 (TWD)</div>
          <div className="text-3xl font-bold text-gray-800 dark:text-white">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden transition-colors">
        <div className="p-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 flex justify-between items-center">
          ...
          <button onClick={onRefresh} className="text-xs text-white bg-green-600 hover:bg-green-700 px-3 py-1.5 rounded-md transition-colors shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {bankData.map(b => (
                <tr key={b.bank} className="bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <input
                      ...
                      onBlur={(e) => {
                        ...
                        e.target.value = formatMoney(parseFloat(e.target.value.replace(/,/g, '')) || 0);
                      }}
              ...
              {bankData.length === 0 && (
                <tr><td colSpan={4} className="p-10 text-center text-gray-400 dark:text-gray-600">目前沒有銀行資料</td></tr>
              )}
            </tbody>
          </table>
        </div>
