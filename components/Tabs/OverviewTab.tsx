...
const OverviewTab: React.FC<OverviewTabProps> = (props) => {
  ...
  const metrics = useMemo(() => {
    ...
    const realizedPnLTWD = portfolioItems.reduce(
      (sum, p) => sum + p.realizedPnL * (props.exchangeRates[p.currency] ?? 1),
      0,
    );
    ...
    const categoryTotals: Record<string, number> = {
      原型: 0,
      槓桿: 0,
      類現金: bankSummary.totalCashTWD,
      其他: 0,
    };
    activeItems.forEach((p) => {
      categoryTotals[p.category] = (categoryTotals[p.category] || 0) + p.marketValueTWD;
    });
    ...
    const totalBeta = activeItems.reduce(
      (sum, p) => sum + p.beta * (p.marketValueTWD / (totalAssets || 1)),
      0,
    );
  ...
  return (
    <div className='animate-fade-in space-y-6'>
      <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
        ...
        <div className='bg-white dark:bg-gray-900 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 transition-colors'>
          <div className='flex justify-between items-center mb-4'>
            ...
            <div className='flex bg-gray-100 dark:bg-gray-800 rounded-lg p-0.5 text-xs'>
              <button
                onClick={() => props.setRateMode('manual')}
                className={`px-2 py-1 rounded-md transition-all ${props.rateMode === 'manual' ? 'bg-white dark:bg-gray-700 shadow text-blue-600 dark:text-blue-400 font-bold' : 'text-gray-500 dark:text-gray-400'}`}
              >
                手動
              </button>
              <button
                onClick={() => props.setRateMode('auto')}
                className={`px-2 py-1 rounded-md transition-all ${props.rateMode === 'auto' ? 'bg-white dark:bg-gray-700 shadow text-blue-600 dark:text-blue-400 font-bold' : 'text-gray-500 dark:text-gray-400'}`}
              >
                自動
              </button>
            </div>
          ...
          <div className='space-y-3'>
            <div className='flex items-center justify-between bg-blue-50 dark:bg-blue-900/20 p-2 rounded-lg border border-blue-100 dark:border-blue-900/40'>
              <label className='text-blue-600 dark:text-blue-400 font-medium text-sm'>
                USD 匯率
              </label>
              <div className='flex items-center gap-2'>
                ...
                <div className='relative'>
                  <input
                    ...
                    value={
                      props.rateMode === 'auto'
                        ? props.exchangeRates.USD.toFixed(2)
                        : props.exchangeRates.USD
                    }
                    onChange={(e) =>
                      props.setExchangeRates({
                        ...props.exchangeRates,
        ...
        <div className='bg-white dark:bg-gray-900 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 transition-colors'>
          ...
          <div className='space-y-2 text-sm max-h-[140px] overflow-y-auto pr-1'>
            ...
            {props.bankData.filter((b) => (b.loan || 0) > 0).length === 0 &&
              pledgeSummary.totalLoan === 0 && (
      ...
      <div className='bg-gradient-to-br from-blue-600 to-blue-800 dark:from-blue-700 dark:to-blue-900 p-6 rounded-xl shadow-md text-center text-white transition-all'>
        <div className='text-blue-100 text-sm uppercase tracking-wider mb-2 font-medium'>
          淨資產總額
        </div>
          ...
          <div>
            ...
            <div className='flex flex-col md:flex-row gap-6 mb-8'>
              ...
              <div className='flex-none w-full md:w-48 bg-gray-900 dark:bg-blue-600 text-white rounded-2xl p-6 shadow-xl flex flex-col items-center justify-center border-b-4 border-blue-500 dark:border-blue-400 transition-all hover:scale-105 duration-300'>
                <div className='text-sm uppercase font-bold tracking-widest opacity-80 mb-2'>
                  Portfolio Beta
                </div>
                <div className='text-5xl font-black font-mono'>{metrics.totalBeta.toFixed(2)}</div>
                <div className='text-[10px] mt-2 px-2 py-0.5 bg-white/20 rounded-full font-bold'>
                  市場敏感度
                </div>
              </div>
              ...
              <div className='flex-1 grid grid-cols-1 sm:grid-cols-3 gap-4'>
                {(Object.entries(metrics.categoryTotals) as [string, number][])
                  ...
                  .map(([name, val]) => (
                    <div
                      key={name}
                      className='p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm flex flex-col justify-center transition-all hover:shadow-md'
                    >
                      ...
                      <div className='text-2xl font-bold dark:text-white font-mono'>
                        {metrics.totalAssets > 0
                          ? ((val / metrics.totalAssets) * 100).toFixed(1)
                          : '0.0'}
                        %
                      </div>
      ...
      <div className='bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden transition-colors'>
        <div className='p-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 flex justify-between items-center'>
          ...
          <button
            ...
            className='text-xs text-white bg-green-600 hover:bg-green-700 px-3 py-1.5 rounded-md transition-colors shadow-sm flex items-center'
          >
              ...
              {showSoldOut &&
                portfolioItems
                  ...
                  .map((p) => (
                    <tr
                      key={p.symbol}
                      className='bg-gray-50 dark:bg-gray-900/50 opacity-60 italic text-gray-400'
                    >
